...
from resources.lib import rbdradio
...
def main_menu():
    items = [
        {'label': plugin.get_string(30000), 'path': "http://69.175.49.133:9994/listen.pls",
         'is_playable': True},
...
def just_in():
    subjects = rbdradio.get_podcasts("/podcasts")
...
def subject_list():
    subjects = rbdradio.get_subjects("/podcasts/subjects")
...
def subject_item(url):
    subjects = rbdradio.podcasts_get(url)
...
def program_menu():
    subjects = rbdradio.get_programs("/podcasts/program")
...
def program_item(url):
    programs = rbdradio.podcasts_get(url)
